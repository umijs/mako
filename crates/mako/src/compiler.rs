...
use crate::utils::id_helper::{assign_numberic_ids, compare_modules_by_incoming_edges};
...
pub struct Context {
    ...
    pub config: Config,
    pub numberic_ids_map: RwLock<HashMap<String, usize>>,
    pub args: Args,
    pub root: PathBuf,
    pub meta: Meta,
...
impl Default for Context {
    fn default() -> Self {
        let mut numberic_ids_map = HashMap::new();
        SWC_HELPERS.iter().enumerate().for_each(|(i, item)| {
            numberic_ids_map.insert(item.to_string(), i);
        });
        ...
        Self {
            ...
            numberic_ids_map: RwLock::new(numberic_ids_map),
        }
    }
}
...
impl Compiler {
    ...
    pub fn compile(&self) -> Result<()> {
        ...
        if let ModuleIdStrategy::Numberic = self.context.config.module_id_strategy {
            let module_graph = self.context.module_graph.read().unwrap();
            assign_numberic_ids(
                module_graph.modules(),
                |a, b| compare_modules_by_incoming_edges(&module_graph, &a.id, &b.id),
                |module, id| {
                    let mut numberic_ids_map = self.context.numberic_ids_map.write().unwrap();
                    // reserved ten indexes for swc helper and others runtime module
                    numberic_ids_map.insert(module.id.id.clone(), id + 10);
                },
            )
        }

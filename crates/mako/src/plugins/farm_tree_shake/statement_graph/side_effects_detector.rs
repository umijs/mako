use mako_core::swc_common::{Span, Spanned, SyntaxContext};
use mako_core::swc_ecma_ast::{
    ArrowExpr, Class, ClassMember, Decl, ExportDecl, Expr, FnDecl, FnExpr, Key, Module, ModuleDecl,
    ModuleItem, PropName, Script, Stmt, VarDecl, VarDeclOrExpr,
};
use mako_core::swc_ecma_utils::{ExprCtx, ExprExt};
use mako_core::swc_ecma_visit::{noop_visit_type, Visit, VisitWith};

pub(in crate::plugins::farm_tree_shake) struct SideEffectsDetector {
    is_top_level: bool,
...
impl SideEffectsDetector {
    ...
    pub fn has_side_effects(&self) -> bool {
        self.side_effects_span.is_some()
    }
                ...
                let pure_init = match stmt.init {
                    Some(ref init) => match init {
                        VarDeclOrExpr::VarDecl(decl) => {
                            is_pure_var_decl(decl, self.unresolved_ctxt)
                        }
...
impl Visit for SideEffectsDetector {
    noop_visit_type!();
...
pub fn is_pure_var_decl(var: &VarDecl, unresolved_ctxt: SyntaxContext) -> bool {

...
fn to_module_line(
    ...
    let stmt = fn_expr.to_module_fn_expr().unwrap().unwrap().into_stmt();

    let swc_module = SwcModule {
        shebang: None,
        span: DUMMY_SP,
        body: vec![stmt.into()],
    };
    ...
    let l = buf.len();
    let content = String::from_utf8_lossy(&buf[0..l - 2]);

    format!(r#""{}" : {} ,"#, module_id_str, content)
}

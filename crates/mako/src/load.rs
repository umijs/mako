...
use crate::public_path::build_stage_public_path;
...
pub fn handle_asset<T: AsRef<str>>(context: &Arc<Context>, path: T) -> Result<String> {
    ...
    if file_size > context.config.inline_limit.try_into().unwrap() {
        ...
        let build_stage_public_path = build_stage_public_path(&context.config);
        Ok(build_stage_public_path + final_file_name.as_str())
    } else {
        let base64 =
